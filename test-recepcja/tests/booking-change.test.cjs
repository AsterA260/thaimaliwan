...
const ctx=vm.createContext({console,URLSearchParams,AbortController,setTimeout,clearTimeout,
 ...
 fetch:async(url,options)=>{calls.push(options);if(mode==='uncertain')throw Error('NETWORK');return{json:async()=>mode==='bad'?{ok:true,booking_id:'OTHER'}:mode==='rejected'?{ok:false,error:'TERMIN_ZAJETY'}:{ok:true,booking_id:options.body.get('operation')==='release'?'AST-SZ-RELEASE':options.body.get('booking_id'),request_id:options.body.get('request_id'),operation:options.body.get('operation')}};}
});
...
(async()=>{
 ...
 assert.equal((await ctx.r21SendChange({operation:'release',salon:'SZ',data:'2026-10-02',start:'16:00',czas:'90',terapeuta:'Kai'})).ok,true);
 mode='bad';assert.equal((await ctx.r21SendChange({operation:'release',salon:'SZ'})).uncertain,true);
 token='';assert.equal((await ctx.r21SendChange(payload)).error,'BRAK_TOKENA');assert.equal(calls.length,7);
