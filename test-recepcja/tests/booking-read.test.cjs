...
(async()=>{
 ...
 response={ok:true,bookings:[{...pair,status:'BLOKADA',booking_type:'VOUCHER',editable:false,releasable:true}]};assert.equal(await ctx.r1WczytajRezerwacje(true),true);assert.ok(ctx.DATA.SZ.visits.every(v=>v.status==='BLOKADA'&&v.serverReleasable&&!v.serverWritable));
