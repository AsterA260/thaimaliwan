...
const ctx=vm.createContext({
  S:{salon:'SZ',data:'2026-09-22',godzina:'14:00',terapeutka:'Kai, Tip',imie:'TEST REZERWACJA',tel:'',usluga:'Para',min:60,cena:380,bucket:'couples'},
...
(async()=>{
  ...
  assert.equal(calls[0].options.body.get('terapeuta'),'Kai, Tip');assert.equal(calls[0].options.body.get('category'),'couples');assert.equal(calls[0].options.body.get('expected_price'),'380');
